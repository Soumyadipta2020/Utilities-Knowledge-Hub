...
def restructure_tab(tab_id, tab_title, tab_desc, process_title, process_html, explanation, dashboard_title, dashboard_html, feature_title, feature_html, chart_title, chart_id):
    feature_section = ""
    if feature_title:
        feature_section = f"""<h4 class="tab-section-title" style="margin-top: 12px;">{feature_title}</h4>
                    <div style="overflow-y: auto; padding-right: 4px;">
                        {feature_html}
                    </div>"""

    return f"""<div id="{tab_id}" class="tab-content" style="padding: 12px 24px; overflow: hidden; flex-direction: column; height: 100%; box-sizing: border-box;">
            <h3 style="font-family: 'Outfit', sans-serif; font-size: 18px; color: #fff; margin: 0; flex-shrink: 0;">{tab_title}</h3>
            <p style="color: var(--text-muted); font-size: 11.5px; margin-top: 4px; margin-bottom: 12px; flex-shrink: 0;">{tab_desc}</p>
            
            <div style="display: flex; gap: 24px; flex: 1; min-height: 0; overflow: hidden;">
                <!-- Left Column -->
                <div style="flex: 1.5; display: flex; flex-direction: column; min-height: 0; overflow: hidden;">
                    <h4 class="tab-section-title" style="margin-top: 0; flex-shrink: 0;">{process_title}</h4>
                    {process_html}
                    
                    <div class="explanation-text" style="margin-bottom: 12px; font-size: 11px; flex-shrink: 0;">
                        {explanation}
                    </div>
                ...
                <div style="flex: 1; display: flex; flex-direction: column; min-height: 0; overflow: hidden;">
                    ...
                    <div style="overflow-y: auto; padding-right: 4px; padding-bottom: 8px;">
                        {dashboard_html}
                    </div>
                    {feature_section}
                </div>
...
tab_info = restructure_tab(
    "tab-info", "Information Harnessing", "Ingestion, parsing, and structured translation of multi-modal enterprise data sources.",
...
tab_knowledge = restructure_tab(
    "tab-knowledge", "Knowledge Harnessing", "Entity extraction, relationship mapping, and enterprise ontology building.",
    "Semantic Workflow",
    """<div class="process-flow" style="flex-shrink: 0;">
                <div class="process-step">
                    <h4>1. Chunking</h4>
...
tab_inference = restructure_tab(
    "tab-inference", "Inference Harnessing", "LLM Reasoning, vector search, and agentic multi-step task execution.",
...
tab_outcome = restructure_tab(
    "tab-outcome", "Outcome Harnessing", "Business value metrics, ROI tracking, and resolution impact.",
...
tab_benchmarking = restructure_tab(
    "tab-benchmarking", "Benchmarking", "Continuous evaluation of response accuracy against ground truth datasets.",
    "Evaluation Pipeline",
    """<div class="process-flow" style="flex-shrink: 0;">
                <div class="process-step">
                    <h4>1. Golden Data</h4>
...
tab_storage = restructure_tab(
    "tab-storage", "Storage", "Infrastructure health, database utilization, and vector cluster status.",
    "Data Topology",
    """<div class="process-flow" style="flex-shrink: 0;">
                <div class="process-step">
...
# Replace the HTML block from tab-info to the end
pattern = re.compile(r'<div id="tab-info".*?</main>', re.DOTALL)
replacement = f"{tab_info}\n{tab_knowledge}\n{tab_inference}\n{tab_outcome}\n{tab_benchmarking}\n{tab_storage}\n    </main>"
content = re.sub(pattern, replacement, content)

# Also update CSS for dashboard-grid, gap and padding to fit in smaller spaces
content = content.replace('minmax(200px, 1fr)', 'minmax(130px, 1fr)')
content = content.replace('padding: 10px;\n            flex: 1;', 'padding: 8px;\n            flex: 1;')
content = content.replace('gap: 10px;\n            margin: 10px 0;', 'gap: 8px;\n            margin: 12px 0;')

with open('app/templates/index.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("Two-column restructuring applied successfully.")
