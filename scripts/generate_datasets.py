...
dates = pd.date_range(start=start_date, periods=WEEKS*7, freq='D')
...
weekly_leads = generate_weekly_counts(BASE_LEADS, WEEKS)
weekly_appts = generate_weekly_counts(BASE_APPTS, WEEKS)
weekly_sales = generate_weekly_counts(BASE_SALES, WEEKS)
weekly_installs = generate_weekly_counts(BASE_INSTALLS, WEEKS)
weekly_repairs = generate_weekly_counts(BASE_REPAIRS, WEEKS)
weekly_services = generate_weekly_counts(BASE_SERVICES, WEEKS)
...
future_dates = pd.date_range(start=end_date + timedelta(days=1), periods=12*7, freq='D')
all_forecast_dates = dates.append(future_dates)
